...
# Lấy env vars trước khi import để có thể ghi lỗi vào Redis nếu import fail
...
def write_error_to_redis(error_msg: str):
    try:
        ...
        print(f"[MANOMANO] Error written to Redis: {error_msg}")
    except Exception as redis_err:
        print(f"[MANOMANO] Could not write error to Redis: {redis_err}")

try:
    sys.path.insert(0, '/app/sourceCode')
    from main import process_single_request
except Exception as import_err:
    print(f"[MANOMANO] Import error: {import_err}")
...
async def main():
    ...
    try:
        ...
        print(f"[MANOMANO] Exception: {e}")

    print(f"[MANOMANO] status={result.get('status')} http={result.get('http_code')} error={result.get('error')} html_len={len(result.get('html') or '')}")
    r.setex(f"result:{ret_key}", result_ttl, json.dumps(result, ensure_ascii=False, default=str))
    print(f"[MANOMANO] Done: {ret_key}")
