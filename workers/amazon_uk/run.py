...
def write_error_to_redis(error_msg: str):
    try:
        ...
        print(f"[AMAZON_UK] Error written to Redis: {error_msg}")
    except Exception as redis_err:
        print(f"[AMAZON_UK] Could not write error to Redis: {redis_err}")
...
try:
    ...
    print(f"[AMAZON_UK] Import error: {import_err}")
...
async def main():
    ...
    try:
        ...
        print(f"[AMAZON_UK] Exception: {e}")

    print(f"[AMAZON_UK] status={result.get('status')} http={result.get('http_code')} error={result.get('error')} html_len={len(result.get('html') or '')}")
    r.setex(f"result:{ret_key}", result_ttl, json.dumps(result, ensure_ascii=False, default=str))
    print(f"[AMAZON_UK] Done: {ret_key}")
